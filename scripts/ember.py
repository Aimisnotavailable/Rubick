...
class Ember:
    ...
    def raw_feature_iterator(self, file_paths):
        """
    ...
    def vectorize(self, irow, raw_features_string, X_path, y_path, extractor, nrows):
        """
    ...
    def vectorize_unpack(self, args):
        """
    ...
    def vectorize_subset(self, X_path, y_path, raw_feature_paths, extractor, nrows):
        """
        ...
        argument_iterator = ((irow, raw_features_string, X_path, y_path, extractor, nrows)
                            for irow, raw_features_string in enumerate(self.raw_feature_iterator(raw_feature_paths)))
        for _ in tqdm.tqdm(pool.imap_unordered(self.vectorize_unpack, argument_iterator), total=nrows):
            pass


    def create_vectorized_features(self, data_dir, feature_version=2):
        """
        ...
        self.vectorize_subset(X_path, y_path, raw_feature_paths, extractor, nrows)
    ...
    def read_vectorized_features(self, data_dir, subset=None, feature_version=2):
        """
    ...
    def create_metadata(self, data_dir):
        """
        ...
        train_records = list(pool.imap(self.read_metadata_record, self.raw_feature_iterator(train_feature_paths)))
        ...
        test_records = list(pool.imap(self.read_metadata_record, self.raw_feature_iterator(test_feature_paths)))
    ...
    def read_metadata(self, data_dir):
        """
    ...
    def optimize_model(self, data_dir):
        """
        Run a grid search to find the best LightGBM parameters
        """
        # Read data
        X_train, y_train = self.read_vectorized_features(data_dir, subset="train")
    ...
    def train_model(self,data_dir, params={}, feature_version=2):
        """
        ...
        # Read data
        X_train, y_train = self.read_vectorized_features(data_dir, "train", feature_version)
